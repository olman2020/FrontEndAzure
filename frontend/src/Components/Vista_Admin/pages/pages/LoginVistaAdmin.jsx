...
function LoginVistaAdmin({ url, img, register, rutathen}) {
 ...
 //envio de la informacion para ser revisada
  const sendInfo = async (data) => {
    try {
      const url="https://apinutritecbd.azurewebsites.net/Administrador/validaradministrador"
      const response= await axios.post(url, {
        correoelectronico: data.gmail,   
        contrasena: md5(data.password)
      });
      if(response.data!==0){
          localStorage.setItem("correo",data.gmail)
          setshowAlert(true)
          navigate( '/administrador/vistasecundaria' );
        alert('Se ha loguiado correctamente')
...
export default LoginVistaAdmin;
